...
public class CalendarAdapter extends BaseAdapter {
	...
	private void UpdateStartDateForMonth() {
		...
		iMonthViewCurrentMonth = nowCalendar.get(Calendar.MONTH);// 得到当前日历显示的月

		// 星期一是2 星期天是1 填充剩余天数
		int iDay = 0;
		int firstDay = Calendar.MONDAY;
		int startDay = firstDay;
		if (startDay == Calendar.MONDAY) {
		...
		nowCalendar.add(Calendar.DAY_OF_MONTH, -1);// 周日第一位
	...
	private ArrayList<java.util.Date> getDates() {
		...
		for (int i = 1; i <= 42; i++) {
			alArrayList.add(nowCalendar.getTime());
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		// 设置背景颜色
		if (equalsDate(seleCalendar.getTime(), myDate)) {
			// 选择的
